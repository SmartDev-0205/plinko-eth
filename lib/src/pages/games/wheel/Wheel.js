...
class Wheel extends React.PureComponent {
    constructor(props) {
        ...
        this.resultShowTimeoutId = 0;
        this.resultUntilShowTimeoutId = 0;
        this.onToggleHelp = () => {
            const { toggleHelp, info } = this.props;
            toggleHelp(!info.showHelp);
        };
        this.onPlaceBet = () => {
            const { wheel, addNewBet, placeBet, catchError, showErrorMessage, web3Available, gameState, loggedIn } = this.props;
            ...
            if (canBet.canPlaceBet) {
                ...
                placeBet(num, safeBetValue, gameType)
                    ...
                    this.resultShowTimeoutId = window.setTimeout(() => this.setState({
                        showResult: false,
                    }), 10000);
                })
        ...
        this.onSegmentsChange = (segments) => {
            // TODO: as action
            const { wheel, changeNum } = this.props;
            const { num } = wheel;
    ...
    render() {
        ...
        return (React.createElement(React.Fragment, null,
            React.createElement(Helmet, null,
                React.createElement("title", null, "Wheel - Dicether"),
                React.createElement("meta", { name: "description", content: "Ethereum state channel based Wheel game" })),
            React.createElement(Ui, { nightMode: nightMode, risk: Math.floor(num / 100), segments: num % 100, value: value, maxBetValue: maxBetValue, onValueChange: this.onValueChange, onPlaceBet: this.onPlaceBet, onRiskChange: this.onRiskChange, onSegmentsChange: this.onSegmentsChange, showResult: showResult, disableRiskSegmentUpdate: spinning, result: Object.assign({}, result), showHelp: info.showHelp, onToggleHelp: this.onToggleHelp })));
    }
}
